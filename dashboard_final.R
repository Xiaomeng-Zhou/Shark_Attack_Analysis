...
#########################################################################
ui <- dashboardPage(
  dashboardHeader(title = "Sharks Attacks Analysis"),
  dashboardSidebar(
      sidebarMenu(
      ...
      tabItem(tabName = "map",
              ...
              sidebarLayout(
                ...
                mainPanel(
                  ...
                  uiOutput("map",width = "110%",height = 650),
...
server <- function(input, output) {
 ...
 ############################## Tab"Map Interaction"########################################
  ...
  output$map <- renderUI({
    leafletOutput("default_plot",width = "110%",height = 650)
  })
  ...
  observeEvent(input$Refresh, {
    output$map <- renderUI({
      leafletOutput("dynamic_plot",width = "110%",height = 650)
    })
  ...
  output$downloadData <- downloadHandler(
    filename = function() {
      paste(input$year,input$country,input$species, input$activity,".csv", sep = "_")
    },
    content = function(file) {
      write.csv(shark_subset(), file, row.names = FALSE)
    }
  )
